(() => {
    ...
    document.addEventListener("DOMContentLoaded", (() => {
        const autoHeight = (minusElem, elem) => {
            const headerHeight = document.querySelector(`.${minusElem}`).offsetHeight;
            document.querySelector(`.${elem}`).style.cssText = `min-height: calc(100vh - ${headerHeight}px - 0.6px)`;
        };
        autoHeight("header", "hero__container");
        const mediaQuery = x => {
            x = window.matchMedia("(max-width: 767.98px)");
            const texts = document.querySelectorAll(".inner__text");
            const links = document.querySelectorAll(".inner__text a");
            if (x.matches) texts.forEach((text => text.classList.add("inner__text_hidden"))); else {
                texts.forEach((text => text.classList.remove("inner__text_hidden")));
            ...
            document.addEventListener("click", (e => {
                e.preventDefault();
                const t = e.target;
                if (t.closest(".how .inner a")) {
                    texts[0].classList.remove("inner__text_hidden");
                    t.style.display = "none";
                }
                if (t.closest(".stars .inner a")) {
                    texts[2].classList.remove("inner__text_hidden");
                    t.style.display = "none";
                }
                if (t.closest(".treasures .inner a")) {
                    texts[3].classList.remove("inner__text_hidden");
                    t.style.display = "none";
                }
            }));
        };
